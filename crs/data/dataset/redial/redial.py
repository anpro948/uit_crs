...
from crs.config import DATASET_PATH
from crs.data.dataset.base import BaseDataset
...
class ReDialDataset(BaseDataset):
    ...
    def _merge_conv_data(self, dialog) -> List[Dict]: # return a list of tokenized utt in a dialog
        augmented_convs = []
        last_role = None
        for utt in dialog:
            ...
            if utt['role'] == last_role:
                ...
                # add mỗi utt (đã tokenize) vào augmented_convs
    ...
    def _augment_and_add(self, raw_con_dict) -> List[Dict]:
        augmented_conv_dicts = []
        context_tokens, context_entities, context_words, context_items = [], [], [], []
        entity_set, word_set = set(), set()

        for i, conv in enumerate(raw_con_dict):
